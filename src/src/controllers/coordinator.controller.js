...
// Models - Queries
const { coordinatorQuery, userQuery, roleQuery } = require('../models/index.queries');


module.exports = {
    createCoordinator: async (req, res) => {
        const userExtract = indexHelpers.userHelpers.extractUserDataHelper(req.body)
        const {password, ...coordinator} = indexHelpers.userHelpers.extractCoordinatorDataHelper(req.body)
        let transaction;
        try {
            transaction = await dbConnectionOptions.transaction();
            //TODO Adjust Role
            const [role] = await roleQuery.findRoleQuery({role: "COORDINATOR_ROLE"})
    ...
    updateCoordinator: async (req, res) => {
        ...
        const coordinator = indexHelpers.userHelpers.extractCoordinatorDataHelper(user)
